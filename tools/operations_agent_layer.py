...
The Operations known-evidence index is generated from the same audits/*.md
structured-block convention as the UI and Deployment layers. It is
intentionally separate from the normative ratified index: it records admitted
Operations-standard evidence without changing frozen standard text.
"""
...
def _load_audit_findings() -> list[tuple[Path, dict]]:
    """Load active audit findings using the fleet-wide fenced-block convention."""
...
def build_known_evidence_index() -> list[dict]:
    """Build admitted Operations evidence from active audit findings."""
    ratified_ids = {s["id"] for s in load_ratified_operations_standards()}
    index: list[dict] = []

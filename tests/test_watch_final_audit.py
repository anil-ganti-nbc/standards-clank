...
def test_deployment_known_evidence_is_admitted_deterministically():
    ...
    assert len(entries) == audit["entries"] == 1
    assert entries[0]["standard"] == "STD-DEPLOY-COM-001"
    assert entries[0]["subject"] == "watch-clank"
    assert entries[0]["kind"] == "known_conformance"
    assert entries[0]["source_reference"] == "audits/watch-clank-cross-domain-2026-09-01-final.md"
